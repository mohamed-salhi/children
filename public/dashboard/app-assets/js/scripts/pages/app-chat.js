...
$(function () {
  var chatUsersListWrapper = $('.chat-application .chat-users-list-wrapper'),
    ...
    userProfileToggle = $('.chat-application .users-profile-toggle'),
    userProfileSidebar = $('.users-profile-sidebar'),
    statusRadio = $('.chat-application .users-status input:radio[name=userStatus]'),
    userChats = $('.users-chats'),
  ...
  if (!$.app.menu.is_touch_device()) {
    ...
    // Admin profile left
    if (userProfileSidebar.find('.users-profile-sidebar-area').length > 0) {
      var userScrollArea = new PerfectScrollbar(userProfileSidebar.find('.users-profile-sidebar-area')[0]);
    }
    ...
    userProfileSidebar.find('.users-profile-sidebar-area').css('overflow', 'scroll');
    userChats.css('overflow', 'scroll');
    profileSidebarArea.css('overflow', 'scroll');

    // on users click sidebar close in touch devices
    $(chatsUserList)
      .find('li')
      .on('click', function () {
